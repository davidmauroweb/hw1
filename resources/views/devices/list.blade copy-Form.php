...
<div class="modal fade" id="ComponentModal" tabindex="-1" role="dialog" aria-labelledby="ComponentModalLabel" aria-hidden="true">
  ...
  <div class="modal fade" id="ListModal" tabindex="-1" role="dialog" aria-labelledby="ListModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="ListModalLabel">DETALLE DE COMPONENTES</h5>
        </div>
                    ...
                    <table id="componentsTable" class="table table-sm">
                        ...
                        <tbody>
                            ...
                            <tr style="text-align: center" id="{{$device->device_id}}">
                                ...
                                <td><button data-bs-toggle="modal" data-bs-target="#ListModal" id = 'btnActives' onclick="Fill({{$device->device_id}}, 0)" class='btn btn-success btn-sm'><i class="fa-solid fa-memory" aria-hidden="true" data-toggle="tooltip" data-placement="top" title='Ver Componentes Activos'></i></button></td>
                                <td><button data-bs-toggle="modal" data-bs-target="#ListModal" id = 'btnLow' onclick="Fill({{$device->device_id}}, 1)" class='btn btn-secondary btn-sm'><i class="fa-solid fa-microchip" aria-hidden="true" data-toggle="tooltip" data-placement="top" title='Ver Componentes Reemplazados'></i></button></td>
                                 <td>
...
$.ajax({
    ...
    dataType: "json",
        ...
        if(data.length > 0) {
            $.each(data, function(i, item){
                cards+='<form action="../components-u" method="POST">@csrf<div class="form-row"><div class="col-auto"><label for="hardware_id_edit">COMPONENTE</label><select class="form-control form-control-sm" id="hardware_id_edit" name="hardware_id" required><option value="">SELECCIONE HARDWARE/SOFTWARE</option>';
                cards+='<option value="' + item.hardware_id + '">' + item.hardware_id + '</option>';
